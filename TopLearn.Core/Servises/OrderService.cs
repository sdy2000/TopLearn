...
namespace TopLearn.Core.Servises
{
    public class OrderService : IOrderService
    {
        ...
        public bool IsUserInCourse(string userName, int courseId)
        {
            ...
            return _context.UserCourses.Any(c=>c.CourseId == courseId&&c.CourseId==courseId);
        }

        public DiscountUseType UseDiscount(int orderId, string code)
        {
            var discount = _context.Descounts.SingleOrDefault(d => d.DescountCode == code);
            ...
            if (discount.EdnDate != null && discount.EdnDate >= DateTime.Now)
                return DiscountUseType.ExpierDate;
            ...
            int percent = (order.OrderSum * discount.DescountPercent) / 100;
            ...
            _context.Descounts.Update(discount);
        ...
        public void AddDiscount(Descount descount)
        {
            _context.Descounts.Add(descount);
            _context.SaveChanges();
        }

        public List<Descount> GetAllDiscounts()
        {
            return _context.Descounts.ToList();
        }

        public Descount GetDiscountById(int discountId)
        {
            return _context.Descounts.Find(discountId);
        }

        public void UpdateDiscount(Descount descount)
        {
            _context.Descounts.Update(descount);
            _context.SaveChanges();
        }

        public bool IsExistCode(string code)
        {
            return _context.Descounts.Any(d => d.DescountCode == code);
        }
    }
}
